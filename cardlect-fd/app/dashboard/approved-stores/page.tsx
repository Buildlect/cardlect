...
export default function ApprovedStoresDashboard() {
  ...
  return (
    <DashboardLayout currentPage="dashboard" role="approved-stores">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  <p className="text-muted-foreground text-xs font-semibold mb-1 uppercase tracking-wide">
                    {metric.label}
                  </p>
          ...
          {partnerSchools.map((school) => (
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                <div className="w-12 h-12 rounded-lg flex items-center justify-center text-xl font-bold" style={{ backgroundColor: CARDLECT_COLORS.primary.darker + '20', color: CARDLECT_COLORS.primary.darker }}>
                  🏫
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                {selectedSchoolId === school.id && (
                  <span className="text-xs font-semibold" style={{ color: CARDLECT_COLORS.primary.darker }}>✓ Selected</span>
                )}
              </div>
            </button>
          ))}
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-foreground">Your Approved Stores</h2>
          <button className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-secondary transition-colors font-medium" style={{ color: CARDLECT_COLORS.primary.darker }}>
            <Store size={16} />
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span key={i} style={{ color: i < Math.floor(store.rating) ? CARDLECT_COLORS.warning.main : 'rgba(255,255,255,0.2)' }}>★</span>
                  ))}
                </div>
              </div>
              <p className="text-xs text-muted-foreground mb-3 pb-3 border-b border-border">{store.location}</p>
              <div className="space-y-2 mb-3 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">This Month:</span>
                  <span className="font-semibold text-foreground">₦{store.thisMonthSales.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">Hours:</span>
                  <span className="font-semibold text-foreground">{store.operatingHours}</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-3 px-2 text-muted-foreground font-semibold text-xs">Student</th>
                  <th className="text-left py-3 px-2 text-muted-foreground font-semibold text-xs">School</th>
                  <th className="text-left py-3 px-2 text-muted-foreground font-semibold text-xs">Amount</th>
                  <th className="text-left py-3 px-2 text-muted-foreground font-semibold text-xs">Status</th>
                  <th className="text-left py-3 px-2 text-muted-foreground font-semibold text-xs">Time</th>
                </tr>
              </thead>
              <tbody>
                {recentTransactions.map((txn) => (
                  <tr key={txn.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                    <td className="py-3 px-2 text-foreground font-medium text-xs">{txn.studentName}</td>
                    <td className="py-3 px-2 text-muted-foreground text-xs">{txn.schoolName}</td>
                    <td className="py-3 px-2 text-foreground font-semibold text-xs">₦{txn.amount.toLocaleString()}</td>
                    <td className="py-3 px-2">
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold" style={{ backgroundColor: getTransactionStatusColor(txn.status) + '20', color: getTransactionStatusColor(txn.status) }}>
                        <span className="w-1 h-1 rounded-full" style={{ backgroundColor: getTransactionStatusColor(txn.status) }} />
                        {getTransactionStatusText(txn.status)}
                      </span>
                    </td>
                    <td className="py-3 px-2 text-muted-foreground text-xs">{txn.time}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Updates & Alerts */}
        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all">
          <h3 className="text-lg font-bold mb-4 text-foreground">Updates</h3>
            ...
            {alerts.map((a, i) => {
              ...
              return (
                <div
                  ...
                  style={{ borderColor: a.color + '30', backgroundColor: a.color + '10' }}
                >
                  {/* Alert Icon */}
                  <div className="p-2 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: a.color + '20' }}>
